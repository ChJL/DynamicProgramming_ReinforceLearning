...
class Node:
    def __init__(self, state: State, parent=None):
        ...
        self.parent = parent
        self.children = {}
        self.Q = 0  # 节点最终收益价值 # the final reward that the node get
        self.N = 0  # 节点被访问的次数 # the accessing times of the node

    def weight_func(self, c_param=1.4):
        if self.N != 0:
            # tip： 这里使用了-self.Q 因为子节点的收益代表的是对手的收益
            # tip: using -self.Q, because the reward of the child node  is  the opponent's reward
    ...
    def select(self, c_param=1.4):
        ...
        """
        '''
        according to the condition of child node, choose the best action return
        c_param: using for exploration
        return: best action, the nextnode of the best action
        '''
    ...
    def expand(self):
        ...
        return the child node we just expanded
        '''
        # take the one still untried
    ...
    def update(self, winner):
        ...
        """
        '''
        update the Q and N after simulation
        winner: the winner in the simulation
        '''
        self.N += 1
    ...
    def is_full_expand(self):
        ...
        """
        '''
        check if the node is full expanded
        '''
