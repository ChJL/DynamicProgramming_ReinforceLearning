...
class State:
    ...
    def get_state_result(self):
        ...
        """
        '''
        return the result of board
        if the game is over, then : is_over = True
        if the game is not over, then : is_over = False,  winner = None

        3 types of result of game over: x win, o win, tie
        so winner can have these 3 condition [x, o, None]
        '''
        board = self.board
    ...
    def get_next_state(self, action):
        ...
        """
        '''
        according to action, return a new state
        '''
...
class Game:
    ...
    def __init__(self, state=None):
        if state:
            if state.board.shape[0] != Game.game_size:
                raise Exception("the size used for starting game is wrong")
    ...
    def step(self, action):
        ...
        """
        '''
        take action -> change the current state
        return next state
        '''
        if action:
